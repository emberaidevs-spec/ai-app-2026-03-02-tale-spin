export default async function handler(req, res) {
  try {
    ...
    if (!req.body || !req.body.prompt || !req.body.tone || !req.body.style || !req.body.genre) {
    ...
    const { prompt, tone, style, genre } = req.body;

    const systemPrompt = `Generate a ${genre} ${style} piece with a ${tone} tone based on the following prompt: ${prompt}.`;
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch data from Groq API: ${response.statusText}`);
    }
